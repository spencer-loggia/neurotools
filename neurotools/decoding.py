...
class VarConvND(torch.nn.Module):
    def __init__(self, kernel_size: Union[tuple[int], int], padding: Union[tuple[int], int], in_channels: int,
                 ...
                 dtype=torch.float, generator=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
class PairwiseLoss(torch.nn.Module):
    ...
    def forward(self, X, y, pairwise_weights=None):
        ...
        loss_vec = torch.zeros((self.nclasses,), device=X.device)
        hinge = torch.nn.ReLU()
        for c in classes:
            d = X[y == c]
            # ask each relevant regressor, is c? early index class is always consider "TRUE"
            scores = d[:, c] # positve correct, negative incorrrect
            # this is a hinge loss on each pairwise comparison with this margin.
            if hasattr(self, "margin") and self.margin is not None:
                scores = self.margin - scores
                mscores = hinge(scores)
                scores = -1 * (torch.square(mscores) - .05 * hinge(-scores))  # will flip again
            full_loss = -1 * scores  # self.loss_model(scores, torch.ones_like(scores, device=X.device))
            full_loss = full_loss # incorrect gets extra penalty
            if pairwise_weights is not None:
                class_weights = pairwise_weights[c, c].reshape(
                    (1, pairwise_weights.shape[1]) + tuple([1] * tailing_dim))
                # weight pairwise losses
                class_loss = torch.sum(full_loss * class_weights)
            else:
                class_loss = torch.sum(full_loss)
            loss_vec[c] = class_loss
        if self.use_rebalancing:
            rebalance_unit = torch.softmax(self.rebalance, dim=0)
            n_loss_vec = torch.softmax(loss_vec / torch.sum(torch.abs(loss_vec)), dim=0)
            self.rebalance = .8 * self.rebalance + .2 * n_loss_vec.detach().clone()  # class gets weighted lower the more we correct for it.
            loss_vec = rebalance_unit * loss_vec
...
class SpatialBN(torch.nn.Module):
    ...
    def __init__(self, channels, train=True, device="cpu", *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.to_trian = train
        self.last = None
    ...
    def forward(self, X):
        ...
        bs = X.shape[0]
        iterk = 100
        i = iterk / bs  # batches to 100 examples
        # want running mean at t-100 to contribute .05
        run_discount = .05 ** (1 / i)
        # want memory mean at t-100 to contribute .1
        mem_discount = .5 ** (1 / i)
        if self.to_trian:
...
class ROISearchlightDecoder():
    def __init__(self, atlas: np.ndarray, lookup: dict, set_names: Tuple[str], nonlinear=True, spatial=(64, 64, 64),
        ...
        self.lin_reg_coef = 1e-4 #1e-2 #0.00001  # 0.0001
        self.spatial_reg_coef = 1e-7 # 1e-5 # 1e-7 #0.0001  # 0.0001
    ...
    def initialize_params(self):
        ...
        base_chan = max(1, math.log2(self.n_classes * (self.dim - 1)))
        # this padding scheme ensures size remains constant, but we must reverse every layer so we don't get offset from input
        ...
        for l in range(self.n_layers):
            ...
            if l == 0:
                ...
                self.bn_layers.append({s: SpatialBN(device=self.device, channels=self.channels) for s in self.set_names})
            else:
                self.bn_layers.append({s: torch.nn.BatchNorm3d(num_features=in_chan, device=self.device, dtype=torch.float) for s in self.set_names})
    ...
    def global_step(self, spatial_logits, top30=False):
        ...
        if self._train_model:
            ...
            spatial_logits = spatial_logits # / (torch.abs(spatial_logits) + mod)
        ...
        # don't condider identity comparisons
        diag_mask = torch.logical_not(torch.eye(self.n_classes, device=self.device)).view(1, self.n_classes,
                                                                                          self.n_classes, 1)
        ...
        if self._train_model:
            ...
            reg += 5 * self.lin_reg_coef * torch.sum(torch.square(spatial_logits))
        if self._train_mask:
    ...
    def forward(self, stim, sess=None, top30=False):
        ...
        logits, reg = self.global_step(spatial_logits, top30=top30)  # <n, c, c, x, y, z>
        return logits, reg
    ...
    def fit(self, dataloader, lr=.01):
        ...
        loss_fxn = PairwiseLoss(nclasses=self.n_classes, device=self.device, rebalance=True, margin=1.0)
        loss_history = []
        ...
        #     bn_params += list(bn[self._train_set].parameters())
        if self._train_model and self._train_mask:
            # set optim to use
            params = search_params + [self.roi_weights[self._train_set]] + bn_params + sess_params
            ...
            params = search_params + bn_params + sess_params
        else:
        ...
        for i, res in enumerate(dataloader):
            ...
            loss = loss + loss_fxn(probs, target, pairwise_weights=self.pairwise_weights)
            probs = probs.mean(dim=3)
            if self._train_mask:
                # need to scale by number of spatial
                loss = loss + self.total_features * loss_fxn(probs, target, pairwise_weights=self.pairwise_weights)
            loss = loss + reg
    ...
    def predict(self, dataloader, top30=False):
        ...
        pairloss = PairwiseLoss(self.n_classes, device=self.device, rebalance=False)
        for i, res in enumerate(dataloader):
            if len(res) == 3:
                stim, target, sess = res
                if self.sessions is None:
            ...
            stim = torch.nn.Parameter(stim)
            target = torch.from_numpy(target)
            target = target.long().to(self.device)
            sgy_hat, reg = self.forward(stim, top30=False, sess=send_sess)
            gy_hat = sgy_hat.mean(dim=3)
            with torch.no_grad():
                ...
                _, acc_map = util.confusion_from_pairwise(sgy_hat, target, self.n_classes,
                                                          pairwise_weights=self.pairwise_weights)
            ...
            spatial_sals.append(sal * stim.data.detach().cpu().numpy())
        ...
        sal_map = np.concatenate(spatial_sals).mean(axis=0)
        sal_map = sal_map.max(axis=0) # take max along channel dimension (trs)
