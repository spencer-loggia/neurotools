...
revnet = models.ReverbNetwork(structure, input_node=0, node_shape=(1, 3, 28, 28))
...
revnet_decoder = torch.nn.Conv2d(kernel_size=28, in_channels=1, out_channels=10)
present_frames = 3
optimizer = torch.optim.Adam(lr=.01, params=list(revnet.parameters()) + list(revnet_decoder.parameters()))
...
for epoch in range(10):
    for i, (stim, target) in enumerate(train_loader):
        ...
        for i in range(present_frames):
            ...
            revnet()
        decode_input = revnet.architecture.nodes[1]['state'][:, 0, :, :][:, None, :, :].clone()
